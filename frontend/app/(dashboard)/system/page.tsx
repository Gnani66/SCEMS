...
const FLOW = [
  { label: "Sensors", sub: "ESP32 + virtual nodes", color: "#4ADE80" },
  { label: "MQTT", sub: "Mosquitto broker", color: "#60A5FA" },
  { label: "Backend", sub: "FastAPI pipeline", color: "#A78BFA" },
  { label: "Database", sub: "Supabase PostgreSQL", color: "#F5B942" },
  { label: "Realtime", sub: "WebSocket stream", color: "#34D399" },
  { label: "Dashboard", sub: "Next.js UI", color: "#F87171" },
];

const NODE_STATUS: Record<string, { label: string; color: string }> = {
  healthy: { label: "Healthy", color: "#4ADE80" },
  running: { label: "Connected", color: "#4ADE80" },
  connected: { label: "Connected", color: "#4ADE80" },
  online: { label: "Connected", color: "#4ADE80" },
  active: { label: "Connected", color: "#4ADE80" },
  disconnected: { label: "Disconnected", color: "#EF4444" },
  unavailable: { label: "Temporarily unavailable", color: "#F5B942" },
  unknown: { label: "Unknown", color: "#929797" },
...
export default function SystemPage() {
  ...
  const wsState =
    connection === "connected"
      ? { label: "Connected", color: "#4ADE80" }
      : connection === "reconnecting"
        ? { label: "Reconnecting", color: "#F5B942" }
        : { label: "Disconnected", color: "#EF4444" };
  const nodeState = liveCount === nodes.length && nodes.length > 0
    ? { label: `${liveCount} / ${nodes.length} Online`, color: "#4ADE80" }
    : nodes.length > 0
      ? { label: `${liveCount} / ${nodes.length} Online`, color: "#F5B942" }
      : { label: "Awaiting nodes", color: "#929797" };

  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-[20px] font-semibold tracking-tight text-ink">System Health</h1>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {loading && !data ? (
          Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-line bg-card p-3.5"><Skeleton lines={2} /></div>
          ))
        ) : (
          <>
            <ServiceTile icon={<IconSystem size={17} />} label="FastAPI Backend" detail={apiState.label} color={apiState.color} pulse={apiState.color === "#4ADE80"} />
            <ServiceTile icon={<IconDatabase size={17} />} label="Supabase Database" detail={dbState.label} color={dbState.color} pulse={dbState.color === "#4ADE80"} />
            <ServiceTile icon={<IconSignal size={17} />} label="MQTT Broker" detail={mqttState.label} color={mqttState.color} pulse={mqttState.color === "#4ADE80"} />
            <ServiceTile icon={<IconWifi size={17} />} label="WebSocket Realtime" detail={wsState.label} color={wsState.color} pulse={wsState.color === "#4ADE80"} />
            <ServiceTile icon={<IconChip size={17} />} label="Sensor Nodes" detail={nodeState.label} color={nodeState.color} pulse={nodeState.color === "#4ADE80"} />
          </>
        )}
      </div>
      ...
      <Card kicker="Data pipeline" title="Architecture" dense
        right={<span className="text-[11px] text-muted">virtual sensors → real dashboard</span>}
      >
        <div className="flex flex-col items-stretch gap-2 p-3 lg:flex-row lg:items-center">
          {FLOW.map((step, i) => (
            <div key={step.label} className="flex flex-1 items-center gap-2">
              <div className="flex flex-1 flex-col gap-1 rounded-lg border border-line bg-app2 px-3 py-2.5 lg:items-center">
      ...
      <div className="grid gap-4 xl:grid-cols-12">
        ...
        <section className="xl:col-span-5">
          <Card kicker="Platform" title="SCEMS Platform" dense>
            ...
            <div className="flex items-center justify-between border-t border-line bg-app2 px-3 py-2.5">
              <span className="flex items-center gap-1.5 text-[11px] text-muted">
                <IconHealth size={13} />
                API version
              </span>
              <span className="text-tabular font-mono text-[12px] text-info">
                0.2.0
              </span>
            </div>
          </Card>
